...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $chk = $conn->query("SELECT id FROM blogs WHERE slug = '$slug' AND id != $id");
    ...
    $featured_img_path = $blog['featured_image'];
    ...
    try {
        $sql = "UPDATE blogs SET 
            ...
            seo_title = ?, seo_description = ?, seo_keywords = ?, status = ?, updated_at = NOW()
            WHERE id = ?";
        ...
        $stmt->bind_param("ssssssssssi", 
            ...
            $seo_title, $seo_description, $seo_keywords, $status, $id
        );
        ...
        // Refresh local blog representation
        $blog['title'] = $title;
        $blog['slug'] = $slug;
        ...
        $blog['seo_description'] = $seo_description;
        $blog['seo_keywords'] = $seo_keywords;
        $blog['status'] = $status;
    } catch (Exception $e) {
        $conn->rollback();
...
<head>
    <meta charset="UTF-8">
    <title>Edit Blog - Amadika Admin</title>
    <!-- Assets -->
        ...
        <div id="page-content-wrapper">
            ...
            <div class="container-fluid mt-4">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h2 class="h3 fw-bold text-secondary">Edit Blog Post: <?php echo htmlspecialchars($blog['title']); ?></h2>
                        ...
                        <div class="tab-content">
                            ...
                            <div class="tab-pane fade show active" id="general">
                                <div class="row">
                                    <div class="col-md-8 mb-3">
                                        ...
                                        <input type="text" class="form-control" name="title" id="blogTitle" required onkeyup="generateSlug(this.value)" value="<?php echo htmlspecialchars($blog['title']); ?>">
                                    </div>
                                    <div class="col-md-4 mb-3">
                                        <label class="form-label">Status <span class="text-danger">*</span></label>
                                        <select class="form-select" name="status" required>
                                            <option value="active" <?php echo $blog['status'] == 'active' ? 'selected' : ''; ?>>Active (Published)</option>
                                            <option value="inactive" <?php echo $blog['status'] == 'inactive' ? 'selected' : ''; ?>>Inactive (Draft)</option>
                                        </select>
                                    </div>
                                    <div class="col-md-8 mb-3">
                                        <label class="form-label">Slug</label>
                                        <input type="text" class="form-control" name="slug" id="slug" value="<?php echo htmlspecialchars($blog['slug']); ?>">
                                    </div>
                                    <div class="col-md-4 mb-3">
                                        <label class="form-label">Author</label>
                                        <input type="text" class="form-control" name="author" placeholder="Admin" value="<?php echo htmlspecialchars($blog['author'] ?: 'Admin'); ?>">
                                    </div>
                                    <div class="col-md-12 mb-3">
                                        <label class="form-label">Summary / Excerpt</label>
                                        <textarea class="form-control" name="summary" rows="3" placeholder="Brief summary of the blog post..."><?php echo htmlspecialchars($blog['summary']); ?></textarea>
                                    </div>
                                    <div class="col-md-12 mb-3">
                                        <label class="form-label">Content <span class="text-danger">*</span></label>
                                        <textarea class="form-control" name="content" id="editor" required><?php echo htmlspecialchars($blog['content']); ?></textarea>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="tab-pane fade" id="media">
                                <div class="mb-3">
                                    ...
                                    <small class="text-muted d-block mt-1">Leave empty to keep current image. PNG, JPG, JPEG, WEBP.</small>
                                    
                                    <div class="mt-3">
                                        <span class="d-block mb-1 text-muted">Current Image:</span>
                                        <?php $current_img = !empty($blog['featured_image']) ? '../../' . $blog['featured_image'] : 'https://via.placeholder.com/150'; ?>
                                        <img id="featPreview" class="preview-img" src="<?php echo $current_img; ?>">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="tab-pane fade" id="seo">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" name="seo_title" placeholder="If left blank, blog title will be used." value="<?php echo htmlspecialchars($blog['seo_title']); ?>">
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">SEO Description (Meta Description)</label>
                                    <textarea class="form-control" name="seo_description" rows="3" placeholder="Search engine description (150-160 characters)..."><?php echo htmlspecialchars($blog['seo_description']); ?></textarea>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">SEO Keywords</label>
                                    <input type="text" class="form-control" name="seo_keywords" placeholder="e.g. leather, premium storage, home decor" value="<?php echo htmlspecialchars($blog['seo_keywords']); ?>">
                                </div>
                            </div>
                        </div>
